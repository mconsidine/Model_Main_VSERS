...
runs_inv3 <- c("I1F075-6",  # True expected return = 7.5%          
               "I1F075-6b",
               "I1F075-6c") # True expected return = 6%
...
df_inv3 <- df_inv.all %>% filter(runname %in% runs_inv3) %>% 
  mutate(runname.2lines = factor(runname, levels = runs_inv3, labels = runs_inv_labels3.2lines), #MattC prob here
         runname = factor(runname, levels = runs_inv3, labels = runs_inv_labels3.1line)) #MattC prob here
...
saveRDS(fof.q, "./Data/fof.q.rds") #MattC
...
fof.a <- df.a %>% filter(slgppf!="other") %>%
  ...
  mutate(invassets=entitlement - claims,
         ...
         equityslgtax = equity / slgtax * 100,
         equitygdpshare = equity / gdp *100)
saveRDS(fof.a, "./Data/fof.a.rds") #MattC



fof.a <- readRDS("./Data/fof.a.rds") #MattC
...
fig.equityShare <- pdata %>%
  ...
  scale_colour_manual(values=c(RIG.blue, RIG.green), name = NULL) +
  # legend +
...
nycrf <- read_excel(paste0("./Data/", fn)) %>% select(fyear, value=crfassumed) %>% mutate(value=value*100, series="nycrf") #MattC
glimpse(nycrf)
...
saveRDS(rates.all, "./Data/rates.all.rds") #MattC

rates.all <- readRDS("./Data/rates.all.rds") #MattC
...
#****************************************************************************
## For MFC 2017 low true return and high volatility          ####
#****************************************************************************

runs_inv3 <- c("I1F075-6",
               "I1F075-6b",# True expected return = 7.5%          
               "I1F075-6c") # True expected return = 6%

runs_inv_labels3.1line  <- c("Expected compound return: assumed = 7.5%, true = 7.5%; \nstandard deviation = 12%",
                             "Expected compound return: assumed = 7.5%, true = 6%;    \nstandard deviation = 12%",
                             "Expected compound return: assumed = 7.5%, true = 7.5%; \nstandard deviation = 17.2%")
...
df_inv3 <- df_inv.all %>% filter(runname %in% runs_inv3) %>% 
  mutate(runname = factor(runname, levels = runs_inv3, labels = runs_inv_labels3.1line))
...
fig_shortfall.FR40less <- 
  ...
  scale_x_continuous(breaks = seq(0,30, 5))+ 
  scale_y_continuous(breaks = seq(0,50, 5)) + 
  scale_color_manual(values = c(RIG.blue, RIG.red, "orange") ) +
  labs(x = "Year",
       y = "Probability (%)",
       title = fig.title) + 
  guides(col = guide_legend(title = NULL), shape = guide_legend(title = NULL)) + 
...
# Fig 12
ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig12_shortfall.FR40less.png"),fig_shortfall.FR40less, width=6.5*0.9, height=6*0.9, units="in")
ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig12_shortfall.ERChike.png" ),fig_shortfall.ERC_hike, width=6.5*0.9, height=6*0.9, units="in")
...
# Distribution of funded ratios
pctile.labels <- c("75th percentile", "50th percentile", "25th percentile")
fig.title <- "Distribution of funded ratios \nacross simulations under different scenarios"
fig_response.FRDist <- 
  df_inv2 %>%  select(runname, year, FR.q25, FR.q50, FR.q75) %>% 
  gather(pctile, value, -runname, -year) %>%
  mutate(pctile = factor(pctile, levels = c("FR.q75", "FR.q50", "FR.q25"), labels = pctile.labels)) %>% 
...
# Distribution of ERC rate
fig.title <- "Distribution of employer contribution rates \nacross simulations under different scenarios"
fig_response.ERCDist <- 
  df_inv2 %>%  select(runname, year, ERC_PR.q25, ERC_PR.q50, ERC_PR.q75) %>% 
  gather(pctile, value, -runname, -year) %>%
  mutate(pctile = factor(pctile, levels = c("ERC_PR.q75", "ERC_PR.q50", "ERC_PR.q25"), labels = pctile.labels)) %>% 
...
# Fig 7
ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig7_response.FRDist.png"),fig_response.FRDist, width=15*.6, height=5*.6, units="in")

 
# Fig 9
ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig9_response.ERCDist.png"),fig_response.ERCDist, width=15*.6, height=5*.6, units="in")
#
...
fig.equityShare <-  
fig.equityShare +  theme(legend.justification=c(0,1), legend.position=c(0.01,0.99),
                         legend.background = element_rect(color = "grey",  size=0.5, linetype=1))
fig.equityShare

ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig1_equityShare.png"),      fig.equityShare, width=8*0.75, height=6*0.74, units="in")

# Fig 2
ggsave(paste0(IO_folder, "/Slides_MFC2017/", "fig2_rates_short_priv.png"), fig.rates_short_priv, width=9*0.7, height=8*0.7, units="in")
