...
df_demo_summary_all %>% select(runname, year, ends_with("avg"))%>% filter(year %in% c(1, 30)) %>% print(n = 1e10, na.print=NULL) #MattC na.print
    
df_AL.ratios_all %>% filter(year %in% c( 30)) %>% print(n = 1e10, na.print=NULL) #MattC na.print
