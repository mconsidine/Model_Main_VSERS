...
load(file = paste0(IO_folder, "/Analysis_Ben/Ben_results.RData"))
load(file = paste0(IO_folder, "/Analysis_Ben/Ben_metrics.RData"))
...
#****************************************************************************************************
df_metrics_Ben_y30 %>% colnames
df_metrics_Ben_y30 %<>% select(runname, FR40_y30, FR40_7.5_y30,
...
df_maxChg_Ben_y30  %<>% select(runname, ERC_PR.5yMaxChg_med,  ERC_PR.5yMaxChg_q75,  ERC_PR.5yMaxChg_q90,
                                        ERC_PR.10yMaxChg_med, ERC_PR.10yMaxChg_q75, ERC_PR.10yMaxChg_q90)
df_metrics_Ben_y30 <-  left_join(df_metrics_Ben_y30, df_maxChg_Ben_y30)

tab.runname <- df_metrics_Ben_y30$runname


tab.risk <- df_metrics_Ben_y30 %>%  
...
write.xlsx2(tab.risk, file = paste0(IO_folder,"/", "Analysis_Ben/Ben_Tables.xlsx"), sheetName = "risk_raw", append = F) #MattC append

results_all %>% filter(sim == 1, year == 1)
