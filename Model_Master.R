...
#  paramlist$range_ea  <- c(Global_paramlist$min.age:(paramlist$r.max - 1)) #MattC possible problem here with ret.max>ea "Neg values in imputed"
  paramlist$range_ea  <- c(Global_paramlist$min.age:(max(actives$ea) - 1)) #MattC possible problem here with ret.max>ea; want to preserve r.max?
...
var.display <- c("year",	
  "AL",	# AL: Total Actuarial liability, which includes liabilities for active workers and pensioners.  
  "NC",	  # NC: Normal Cost
