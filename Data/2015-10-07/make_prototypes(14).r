...
# Revisions
#  10/7/2015
#    - change salary growth to yos basis rather than converting from yos to age basis
#    - change actives interpolation so that it is done using grouped data on an age-yos basis rather than
#      using the grouped data after it was converted to age-ea basis; then, after interpolation, convert
#      results to an age-ea basis
...
getactives <- function(plan, dir, fn) {
  ...
  yoscuts <- df %>% filter(type %in% c("yoslb", "yosub")) %>%
    select(type, starts_with("X")) %>%
    gather(yos.cell, value, -type) %>%
    mutate(yos.cell=as.integer(gsub("[^0-9]", "", yos.cell)),
           value=cton(value)) %>% spread(type, value)
  ...
  eacuts <- getcuts(select(df.ea, ea), "ea")

  df.yos <- df2 %>%
    select(planname, age, yos, age.cell, yos.cell, value, type) %>%
    spread(type, value) %>%
    filter(nactives>0)
  ...
  lactives$actives.yos <- df.yos
  lactives$yoscuts <- yoscuts
  lactives$actives.ea <- df.ea
...
getsalgrowth.age <- function(plan, dir, fn) {
  planname <- paste0(toupper(plan), ".age")
...
getsalgrowth.yos <- function(plan, dir, fn) {
  planname <- paste0(toupper(plan), ".yos")
  sheet <- paste0(plan, ".salgrowth")
  range <- xlrange(dir, fn, sheet, "B2", "B3")
  df <- readWorksheetFromFile(paste0(dir, fn), sheet=sheet, header=TRUE, region=range, colTypes="character")
  # If data are on a yos basis convert to age basis (20:70) assuming entrance at 20
  df2 <- df %>% mutate(yos=as.integer(yos), rate=cton(rate))
  maxyos <- max(df2$yos)
  ...
  df3 <- left_join(data.frame(yos=0:60), df2) %>%
    ...
    select(planname, yos, salgrowth=rate)
  return(df3)
}
...
fillin.actives <- function(lactives, sgdf, inflation) {
  ...
  adf <- select(lactives$actives.ea, planname, age, ea, nactives, salary) # keep only the vars we want
...
# now make long file
ptermrates <- df %>% gather(planname, termrate, -yos) %>%
  mutate(termrate=cton(termrate), planname=as.character(planname), yos=as.integer(yos)) %>%
  filter(!is.na(termrate)) %>%
  select(planname, yos, termrate)
...
plan.termrates <- ldply(1:length(biglist), function(lnum) return(biglist[[lnum]]$termrates))
proto.termrates <- readRDS(paste0(dir, "proto_termrates.rds"))
termrates <- bind_rows(plan.termrates, proto.termrates)

retrates <- readRDS(paste0(dir, "proto_retrates.rds"))
...
salgrowth %>% qplot(yos, salgrowth, data=., colour=planname, geom=c("point", "line"))
